...
//   });
  ...
  async function transitStateThirdPartyDID() {
    ...
    core.registerDidMethodNetwork({
      ...
      blockchain: 'BBN',
      network: 'Hyderabad',
      networkFlag: 0b01000001 | 0b00001001,
      chainId: 2023
    });
  ...
  core.registerDidMethodNetwork({
    ...
    blockchain: 'BBN',
    network: 'Hyderabad',
    networkFlag: 0b11000001 | 0b00001011
  });
  ...
  // const blockchain = core.Blockchain.linea;
  // const networkId = core.NetworkId.test;
  const blockchain = core.Blockchain.BBN;
  const networkId = core.NetworkId.Hyderabad;
  const { did: userDID } = await identityWallet.createIdentity({
    method,
    blockchain,
    networkId,
  ...
  const { did: issuerDID } = await identityWallet.createIdentity({
    ...
    // blockchain: core.Blockchain.linea,
    // networkId: core.NetworkId.test,
    blockchain: core.Blockchain.BBN,
    networkId: core.NetworkId.Hyderabad,
    revocationOpts: {
      type: CredentialStatusType.Iden3ReverseSparseMerkleTreeProof,
      id: rhsUrl
    }
...
async function main(choice: string) {
  switch (choice) {
    ...
    default:
      ...
      await issueCredential();
      await transitState();
      await transitStateThirdPartyDID(); // -- Error while publishing to BBN Testnet. 
      await generateProofs();
      await handleAuthRequest();
      await handleAuthRequestWithProfiles();
      await handleAuthRequestWithProfilesV3CircuitBeta();
